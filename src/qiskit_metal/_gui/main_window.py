...
# Shared icon edge length for the two top toolbars. The .ui shipped
# mismatched sizes (24px implicit on toolBarDesign, 32px hardcoded on
# toolbar_renderers); one value keeps them the same height.
_TOOLBAR_ICON_PX = 20
...
class MetalGUI(QMainWindowBaseHandler):
    ...
    def _ui_adjustments(self):
        ...
        # Compact the top toolbars. The .ui sets ToolButtonTextUnderIcon on
        # toolBarDesign, stacking each label below its icon, and gives
        # toolbar_renderers a hardcoded 32px icon -- between them the top of
        # the window ate roughly twice the height it needed, and the two bars
        # did not even match. Beside-icon at a single shared size keeps every
        # label readable and lets both bars sit tight against each other.
        # Set here rather than in the .ui because ``main_window_ui.py`` is
        # generated by pyside6-uic and regenerated from the .ui on demand.
        for _toolbar in (self.ui.toolBarDesign, self.ui.toolbar_renderers):
            _toolbar.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
            _toolbar.setIconSize(QSize(_TOOLBAR_ICON_PX, _TOOLBAR_ICON_PX))
            # Drop the default frame padding so the bar hugs its buttons.
            _toolbar.setContentsMargins(0, 0, 0, 0)
            if _toolbar.layout() is not None:
                _toolbar.layout().setSpacing(2)
                _toolbar.layout().setContentsMargins(2, 0, 2, 0)
